...
var BookmarkWhere = struct {
	...
	FQDN:      whereHelperstring{field: "`bookmark`.`fqdn`"},
...
var (
	bookmarkAllColumns            = []string{"user_id", "fqdn", "created_at"}
	bookmarkColumnsWithoutDefault = []string{"user_id", "fqdn", "created_at"}
	bookmarkColumnsWithDefault    = []string{}
	bookmarkPrimaryKeyColumns     = []string{"user_id", "fqdn"}
)
...
func FindBookmark(ctx context.Context, exec boil.ContextExecutor, userID uint64, fQDN string, selectCols ...string) (*Bookmark, error) {
	...
	query := fmt.Sprintf(
		"select %s from `bookmark` where `user_id`=? AND `fqdn`=?", sel,
	)

	q := queries.Raw(query, userID, fQDN)
...
func (o *Bookmark) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `bookmark` WHERE `user_id`=? AND `fqdn`=?"
...
func (o *Bookmark) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindBookmark(ctx, exec, o.UserID, o.FQDN)
	if err != nil {
		return err
	}
...
func BookmarkExists(ctx context.Context, exec boil.ContextExecutor, userID uint64, fQDN string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `bookmark` where `user_id`=? AND `fqdn`=? limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, userID, fQDN)
	}
	row := exec.QueryRowContext(ctx, sql, userID, fQDN)
